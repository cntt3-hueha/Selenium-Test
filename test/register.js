...
let { email, pass, full_name_required_text } = require("../credentials.json");
...
describe('Test User Register', function () {
    ...
    let emailData = 'ngaha1999+2@gmail.com';
    ...
    // it('Test Case #1- Verify user click on the register button', async function () {
    ...
    //     // var actualValue = username.getAttribute('required')
    //     var actualFullNameActivationMessage = await username.getAttribute("validationMessage");
    //     // if (actualFullNameActivationMessage != full_name_required_text) {
    //     //     throw Error('Not pass');
    //     // }
    //     // var isValid = await driver.executeScript("return arguments[0].checkValidity();", username)
    //     var isValid = await driver.executeScript("return arguments[0].validity.valid;", username)
    //     // var isFormInvalid = await driver.executeScript("return arguments[0].validity.valid;", username)
    //     if (isValid != false || actualFullNameActivationMessage != 'Vui lòng điền vào trường này.') throw Error(actualFullNameActivationMessage);
    // });

    // it('Test Case #2- Input text into fullname field', async function () {
    ...
    //     if (isValid != false || actualNumberActivationMessage != 'Vui lòng nhập số.') throw Error(actualNumberActivationMessage);
    // });
    ...
    it('Test Case #6 - Paste text into fullname field', async function () {
        ...
        submitBtn.click();
        await driver.sleep(2000);
        driver.wait(until.urlContains('dang-nhap'), 1000);
